 ...
 */
@Serializable
data class MobTemplate(val model: Int,
                       ...
                       @SerialName("is-adult") val isAdult: Boolean = true,
                       @SerialName("death-commands") val deathCommands: List<String> = ArrayList(),
                       val drops: List<MobDrop> = listOf()) {
    val name by lazy {
        (mobzy.mobzyTypes.templates.entries.find { (_, template) -> template === this }
                ?: error("Template accessed but not registered under mobzy anymore")).key
    }
